...
const ShowNFT = () => {
  ...
  useEffect(() => {
    const fetchNFTDetails = async () => {
      try {
        const id = typeof nft === 'object' ? nft._id : nft
        if (!id) return

        const res = await fetch(`http://localhost:5000/api/nfts/${id}`)
  ...
  const handleNFTPurchase = async () => {
    ...
    setGlobalState('loading', {
      show: true,
      msg: 'Transferring NFT ownership...',
    })

    try {
      const res = await fetch(`http://localhost:5000/api/nfts/${nftData._id}/transfer`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ newOwner: connectedAccount, price: nftData.price }),
      })

      if (!res.ok) throw new Error('Ownership transfer failed')

      const updatedNFT = await res.json()
      setNftData(updatedNFT.nft)
      ...
      <div className="bg-[#151c25] shadow-xl shadow-[#e32970] rounded-xl w-11/12 md:w-2/5 h-7/12 p-6">
        <div className="flex flex-col">
          ...
          <div className="flex flex-col justify-start rounded-xl mt-5">
            <h4 className="text-white font-semibold">{nftData.title}</h4>
            <p className="text-gray-400 text-xs my-1">{nftData.description}</p>

            <div className="flex justify-between items-center mt-3 text-white">
              <div className="flex justify-start items-center">
                <Identicon
                  string={nftData.owner}
                  size={50}
              ...
              <div className="flex flex-col">
                ...
                <p className="text-sm font-semibold">{nftData.price} ETH</p>
              </div>
            </div>
          </div>

          <div className="flex justify-between items-center space-x-2">
            {connectedAccount === nftData.owner ? (
              <button
